...
## Hermes Integration via delegate_task (ACP模式)

**这是推荐模式（方案B）** — 日常对话走DeepSeek省钱，复杂金融分析派Claude Code干活。

### 前提条件

1. Claude Code已安装: `npm install -g @anthropic-ai/claude-code`
2. 已设置 `ANTHROPIC_API_KEY` 环境变量
3. 运行 `claude doctor` 确认健康可用

### 配置 delegate_task

在 Hermes config.yaml 中配置 ACP 命令：
...
或者在调用 `delegate_task` 时直接指定：

```python
# 派子Agent走Claude Code
delegate_task(
    tasks=[{"context": "...", "goal": "..."}],
    acp_command="claude"  # ← 关键参数
)
```

### 适用场景

| 场景 | 用DeepSeek | 派Claude Code |
|------|-----------|---------------|
| 日常聊天、简单问答 | ✅ 默认 | ❌ 浪费 |
| 查看股票行情、K线 | ✅ | ❌ 不需要 |
| 建DCF估值模型、Comps分析 | ❌ 不够好 | ✅ 原生支持 |
| 做PPT投标书 | ❌ | ✅ 懂投行模板 |
| 运行66个Anthropic金融框架技能 | ❌ 需手工适配 | ✅ 原生理解 |
| 下载SEC Filing并解析 | ✅ | ✅ 都行 |

### 最佳实践

- Claude Code print模式: `claude -p "分析NVDA的DCF估值" --allowedTools "Read,Bash" --max-turns 15`
- Claude Code交互模式: 通过tmux启动多轮会话
- 复杂任务派Claude干完，结果汇总后由DeepSeek回复给Keke

### 已安装验证

- Claude Code v2.1.143 已安装于腾讯云上海服务器
- npm: 10.9.7
- 待Keke提供 Anthropic API Key 后激活
