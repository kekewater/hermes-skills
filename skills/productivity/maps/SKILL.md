...
'''Calibrate server time from coordinates''' (timezone → sysclock validation):
1. Get coordinates of the server location (or use known lat/lon)
2. `timezone LAT LON` → returns timezone name, UTC offset, current local time
3. `timedatectl status` → check system timezone, NTP sync, local time
4. `chronyc sources` → verify NTP source health
5. Verify: system timezone matches coordinate timezone; local time ≈ timeapi.io response
1. `area "Downtown Seattle"` → get bounding box
2. `bbox S W N E restaurant --limit 30`
...
- ⚠️ **Nominatim blocked from China**: `search`, `reverse`, and `--near` (which auto-geocodes) all use Nominatim, which is unreachable from mainland Chinese servers. The `timezone` command uses timeapi.io (accessible from China) and works fine. For coordinate lookups from China, use the Vultr proxy or an alternative API (e.g., Baidu Maps).
